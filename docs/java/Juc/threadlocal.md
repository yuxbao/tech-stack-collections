...
threadLocal 对象的几乎所有方法，都调用了 threadLocaMap 的相应方法来完成实际的存取操作。
...
ThreadLocalMap 没有实现 Map 接口，而是自定义实现了线性探测哈希表。

`threshold` 是扩容阈值，当 size 达到 threshold 时，就会进⾏扩容。

`set()` 方法中，如果发现哈希冲突，就会线性探测下一个位置，直到找到空位或者 key 相同的位置。
...
private void set(ThreadLocal<?> key, Object value) {
        ...
        for (Entry e = tab[i];
            ...
            if (k == key) { // key存在，更新value
                e.value = value;
                return;
            }

            if (k == null) { // key为null，清理无效Entry
...
`threadLocalHashCode`是一个黄金分割数，每次创建一个 ThreadLocal 对象被分配，可以让==哈希码分布的非常均匀==。
...
线性探测寻找下一个空闲槽位，因为设计初衷就是存放线程私有变量，不会有大量 key，更节省空间。

### 扩容机制

```java
private void rehash() {
    // 元素数量达到阈值，先清理key为null的Entry
    expungeStaleEntries();
...
创建子线程时，会检查父线程有没有 inheritableThreadLocals，有的话就拷贝到子线程。

或者，可以使用优化的 ThreadLocal，阿⾥的 **TransmittableThreadLocal**，不仅实现了⼦线程可以继承⽗线程 ThreadLocal 的功能，并且还可以跨线程池传递值。
